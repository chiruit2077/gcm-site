package br.com.ecc.client.ui.component.richtext;
...
public class RichTextToolbar extends Composite {
	...
	private class EventListener implements ClickListener, ChangeListener, KeyboardListener {
		public void onChange(Widget sender) {
			if (sender == backColors) {
				basicFormatter.setBackColor(backColors.getValue(backColors.getSelectedIndex()));
				backColors.setSelectedIndex(0);
			} else if (sender == foreColors) {
				basicFormatter.setForeColor(foreColors.getValue(foreColors.getSelectedIndex()));
				foreColors.setSelectedIndex(0);
			} else if (sender == fonts) {
				basicFormatter.setFontName(fonts.getValue(fonts.getSelectedIndex()));
				fonts.setSelectedIndex(0);
			} else if (sender == fontSizes) {
				basicFormatter.setFontSize(fontSizesConstants[fontSizes.getSelectedIndex() - 1]);
		...
		public void onClick(Widget sender) {
			if (sender == bold) {
				...
				basicFormatter.toggleItalic();
			} else if (sender == underline) {
				basicFormatter.toggleUnderline();
			} else if (sender == subscript) {
				basicFormatter.toggleSubscript();
			} else if (sender == superscript) {
				basicFormatter.toggleSuperscript();
			} else if (sender == strikethrough) {
				extendedFormatter.toggleStrikethrough();
			} else if (sender == indent) {
				extendedFormatter.rightIndent();
			} else if (sender == outdent) {
				extendedFormatter.leftIndent();
			} else if (sender == justifyLeft) {
				basicFormatter.setJustification(RichTextArea.Justification.LEFT);
			} else if (sender == justifyCenter) {
				basicFormatter.setJustification(RichTextArea.Justification.CENTER);
			} else if (sender == justifyRight) {
				basicFormatter.setJustification(RichTextArea.Justification.RIGHT);
			} else if (sender == insertImage) {
				String url = Window.prompt("Adiciona o link de uma imagem:", "http://");
				if (url != null) {
					extendedFormatter.insertImage(url);
				}
			} else if (sender == createLink) {
				String url = Window.prompt("Adicione o endereço do link:", "http://");
				if (url != null) {
					extendedFormatter.createLink(url);
				}
			} else if (sender == removeLink) {
				extendedFormatter.removeLink();
			} else if (sender == hr) {
				extendedFormatter.insertHorizontalRule();
			} else if (sender == ol) {
				extendedFormatter.insertOrderedList();
			} else if (sender == ul) {
				extendedFormatter.insertUnorderedList();
			} else if (sender == removeFormat) {
				extendedFormatter.removeFormat();
	...
	private RichTextArea.BasicFormatter basicFormatter;
	private RichTextArea.ExtendedFormatter extendedFormatter;
	...
	public RichTextToolbar(RichTextArea richText) {
		...
		this.basicFormatter = richText.getBasicFormatter();
		this.extendedFormatter = richText.getExtendedFormatter();
		...
		if (basicFormatter != null) {
			bottomPanel.add(backColors = createColorList("Cor de fundo"));
			bottomPanel.add(foreColors = createColorList("Cor do texto"));
	...
	private void updateStatus() {
		if (basicFormatter != null) {
			bold.setDown(basicFormatter.isBold());
			italic.setDown(basicFormatter.isItalic());
			underline.setDown(basicFormatter.isUnderlined());
			subscript.setDown(basicFormatter.isSubscript());
			superscript.setDown(basicFormatter.isSuperscript());
		}

		if (extendedFormatter != null) {
			strikethrough.setDown(extendedFormatter.isStrikethrough());
		}
	}
}